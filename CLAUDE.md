...
- `mkdocs.yml` + `docs/Manual/` build the German user manual site (`mkdocs serve`/`mkdocs build` to preview locally), modeled on AutoNum's. Deployed to GitHub Pages by `.github/workflows/manual.yml` on every push to `main` that touches `docs/Manual/**`, `mkdocs.yml`, or the workflow itself, and served at `https://matrikelhelfer.niggl-schlagbauer.de/` (custom domain configured in the repo's Pages settings, not via a repo `CNAME` file — same as AutoNum).
