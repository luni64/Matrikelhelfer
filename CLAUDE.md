...
- `mkdocs.yml` + `docs/Manual/` scaffold the German user manual site (`mkdocs serve`/`mkdocs build` to preview locally), modeled on AutoNum's — **not yet deployed** (no `.github/workflows/manual.yml`). Wire up deployment when the app ships to a wider audience.
