...
└── Views/
    ...
    ├── EventTypeDialog.xaml/.cs # grouped Gramps event-type picker + event date (qualifiers) + description; also edits pending events
    ├── NewPersonDialog.xaml/.cs # name/gender entry for a virtual person from a "Neu" slot; also edits virtual persons
    ├── CitationEditDialog.xaml/.cs # edits a saved find's citation fields (Seite + Kommentar) incl. "Als Kopie speichern"
