...
`CopyableField` is the reusable building block: floating-watermark label + value + copy button (+ optional extra-action button, e.g. open URL / save image). Opt-in DPs: `ValueWrapping` (long rendered text), `IsValueReadOnly=False` (editable fields also need `Mode=TwoWay` on their Value binding), `WarnWhenEmpty` (red border while empty — applied from code-behind as a **local** `BorderBrush` value because MahApps' own style/template triggers silently override a mere style-trigger setter).
