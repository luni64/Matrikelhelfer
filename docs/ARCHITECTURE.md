...
- **Staged corrections of EXISTING Gramps objects** (2026-08 — the additive-only rule of spec §2.2 was consciously and NARROWLY softened; corrections arise while reading the book): persons (name/gender, same dialog as creation), events (type within scope / date / place / description, plus **delete** with ✕-toggle), citations (page/Fundstelle). Everything is a **change entry** — reviewable, removable, uploaded in the ONE transaction — never an immediate write. The batch carries them as **sparse updates** (only changed keys, diffed against the Gramps ORIGINALS — reverting all fields un-stages; null clears) plus `expect_change`, the object's change time as read. Key behaviours:
  - **The view always shows the EFFECTIVE state**: fact rows render staged values as "(korrigiert)" italic, delete-staged rows strike through and dim, person boxes show the effective name italic (tooltip carries "Korrektur vorgemerkt: …"), citation cards preview the corrected page. The change list and tooltips show the delta — display and upload can never drift.
  - **The staleness guard is a race backstop, not a UX step**: `SendAllAsync` auto-reloads every affected person right before building the batch and re-arms all `expect_change` values (`RefreshStagedExpectations`, also run on every detail read incl. the explicit ↻ reload button in the search row). The bridge checks `expect_change` in **step 0, before its own commits** — the batch itself legitimately re-commits targeted objects (adding a parent commits the child), which must not false-CONFLICT.
  - **Open Gramps editors** on touched objects (they hold a stale in-memory copy; saving one later would overwrite the batch): clean editors are closed silently via `uistate.gwm`/`_do_close`, editors with unsaved edits refuse the batch with 409 `EDITOR_OPEN` ("speichern oder verwerfen, dann erneut senden") — typed changes are never discarded. Headless runs have no `uistate`; the check no-ops.
  - **Failures shout**: any upload failure (nothing written, change list intact) and a failed library write raise an error **dialog** — the status bar is neutral-colored informational text only.
