...
- Faster deleting of saved entries: a red trash button appears on each row on hover (one click, no need to select first), and a guarded "Alle löschen" button at the bottom of the list bulk-deletes all entries after a confirmation. Deleting the entry currently shown in the main fields now also clears those fields. The splitter between the fields and the saved-entries panel is now the full gap and draggable across its whole width.
